...
public class StudentRepositoryWithBagRelationshipsImpl implements StudentRepositoryWithBagRelationships {
    ...
    public Optional<Student> fetchBagRelationships(Optional<Student> student) {
        return student.map(this::fetchAttendances);
    }
    ...
    public List<Student> fetchBagRelationships(List<Student> students) {
        return Optional.of(students).map(this::fetchAttendances).orElse(Collections.emptyList());
    }

    Student fetchAttendances(Student result) {
        return entityManager
            .createQuery("select student from Student student left join fetch student.attendances where student is :student", Student.class)
            .setParameter("student", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Student> fetchAttendances(List<Student> students) {
        ...
        List<Student> result = entityManager
            .createQuery(
                "select distinct student from Student student left join fetch student.attendances where student in :students",
                Student.class
            )
