...
type RestOf<T extends ITeacher | NewTeacher> = Omit<T, 'joiningDate'> & {
...
export class TeacherService {
  ...
  protected convertDateFromClient<T extends ITeacher | NewTeacher | PartialUpdateTeacher>(teacher: T): RestOf<T> {
    return {
      ...teacher,
      joiningDate: teacher.joiningDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTeacher: RestTeacher): ITeacher {
    return {
      ...restTeacher,
      joiningDate: restTeacher.joiningDate ? dayjs(restTeacher.joiningDate) : undefined,
    };
  }
