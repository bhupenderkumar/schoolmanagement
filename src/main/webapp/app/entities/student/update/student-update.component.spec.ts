...
describe('Student Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call parentId query and add missing value', () => {
      const student: IStudent = { id: 456 };
      const parentId: IParent = { id: 59652 };
      student.parentId = parentId;

      const parentIdCollection: IParent[] = [{ id: 66184 }];
      jest.spyOn(parentService, 'query').mockReturnValue(of(new HttpResponse({ body: parentIdCollection })));
      const expectedCollection: IParent[] = [parentId, ...parentIdCollection];
      ...
      expect(parentService.addParentToCollectionIfMissing).toHaveBeenCalledWith(parentIdCollection, parentId);
      expect(comp.parentIdsCollection).toEqual(expectedCollection);
    });

    it('Should update editForm', () => {
      const student: IStudent = { id: 456 };
      const parentId: IParent = { id: 72286 };
      student.parentId = parentId;

      activatedRoute.data = of({ student });
      comp.ngOnInit();

      expect(comp.parentIdsCollection).toContain(parentId);
