...
type RestOf<T extends IStudent | NewStudent> = Omit<T, 'admissionDate' | 'tcDate'> & {
...
export class StudentService {
  ...
  protected convertDateFromClient<T extends IStudent | NewStudent | PartialUpdateStudent>(student: T): RestOf<T> {
    return {
      ...student,
      admissionDate: student.admissionDate?.format(DATE_FORMAT) ?? null,
      tcDate: student.tcDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restStudent: RestStudent): IStudent {
    return {
      ...restStudent,
      admissionDate: restStudent.admissionDate ? dayjs(restStudent.admissionDate) : undefined,
      tcDate: restStudent.tcDate ? dayjs(restStudent.tcDate) : undefined,
    };
  }
